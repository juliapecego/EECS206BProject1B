...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    start_pos = np.array([current_position[0], current_position[1], current_position[2]])
    # circle_start_pos = np.array([current_position[0] + 0.2], [current_position[1], 0])
                                       
    end_pos = tag_pos                                    
    if task == 'line':
        end_pos = np.array([tag_pos[0][0], tag_pos[0][1], tag_pos[0][2] + .35])
        trajectory = LinearTrajectory(start_pos, end_pos, 2)
    elif task == 'circle':
        end_pos = np.array([tag_pos[0][0], tag_pos[0][1], tag_pos[0][2] + .35])
        trajectory = CircularTrajectory(start_pos, 0.1, 20)
    elif task == 'polygon':
        for i in range(len(tag_pos)):
            end_pos[i][2] += 0.35
        trajectory = PolygonalTrajectory(end_pos[0], end_pos[1], end_pos[2], end_pos[0], 28)
    else:
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        # YOUR CODE HERE
        #Kp = np.array([0.08, 0.1, 0.8, 0.12, 0.12, 0.12])
        #Kp = np.array([.015, .1, .05, 0.03, 0.03, 0.03])
        #for line Kp = np.array([.005, .001, .003, 0.05, 0.1, 0.1])    #AMIR
        # for line Kv = np.array([0.07, 0.05, 0.05, 0.003, 0.003, 0.003])
        Kp = np.array([.001, .001, .001, 0.01, 0.01, 0.01])    #polyhon
        Kv = np.array([0.07, 0.05, 0.05, 0.003, 0.003, 0.003]) #polygon
        #circle Kp = np.array([.0015, .001, 0.00001, 0.008, 0.008, 0.008])    #AMIR
        #circle Kv = np.array([0.1, 0.05, 0.05, 0.05, 0.05, 0.05])
        #Kv = 2*np.sqrt(Kp)
        #2*sqrt(Kp)
        ...
        # YOUR CODE HERE
        #Kp = np.ones(7) * 1.0
        #Kp = np.array([4, 2.1, 3, 2, 2.2, 2.5, 2])
        Kp = np.array([.9, .9, 1.5, .8, .82, .8, .8])
        #Kv = 2*np.sqrt(Kp)
        Kv = np.array([2.2, 1, 1.3, 1.5, 3.5, 2, 1.2])
        controller = PDJointVelocityController(limb, kin, Kp, Kv)
    elif controller_name == 'torque':
        # YOUR CODE HERE
        #Kp = np.array([405, 210, 400, 1300, 6300, 2600, 50000])
        #Kv = np.array([45, 27, 20, 40, 30, 100, 3800])
        
        #Kp = np.array([100, 180, 1000, 1000, 8000, 1400, 70000])
        #Kv = np.array([30, 20, 15, 10, 10, 40, 30])
        
        Kp = np.array([600, 450, 1600, 1150, 6000, 2300, 85000])
        Kv = np.array([20, 20, 20, 15, 20, 20, 25])
        #Kv = np.array([25, 25, 25, 15, 25, 20, 25])
        #kv[5] <= 50
