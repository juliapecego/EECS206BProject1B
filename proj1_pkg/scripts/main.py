...
def get_trajectory(limb, kin, ik_solver, tag_pos, args):
    ...
    if task == 'line':
        ...
        end_pos = np.array([tag_pos[0][0], tag_pos[0][1], tag_pos[0][2] + .35])
        trajectory = CircularTrajectory(start_pos, 0.2, 20)
    elif task == 'polygon':
...
def get_controller(controller_name, limb, kin):
    ...
    if controller_name == 'workspace':
        # YOUR CODE HERE
        #JUlias Kp = np.array([0.0085, 0.002, 0.01, 0.03, 0.03, 0.03])
        #Kp = np.array([0.001, 0.002, 0.0005, 0.01, 0.01, 0.01])
        #Kp = np.array([.0125, .05, .05, 0.03, 0.03, 0.03])
        #Kp = np.array([.01, .3, .9, 0.1, 0.1, 0.1])    
        Kp = np.array([0.01, 0.001, 0.003, 0.003, 0.003, 0.003])
        Kv = 2*np.sqrt(Kp)
        #2*sqrt(Kp)
        ...
        Kp = np.array([.9, .8, 1, .8, .82, .8, .8])
        #Kv = 2*np.sqrt(Kp)
        Kv = np.array([2.2, 1, 1.5, 1.5, 3.5, 2, 1.2])
        ...
        #Kp = np.array([500, 300, 1300, 1150, 4800, 2000, 80500])
        #Kv = np.array([25, 20, 15, 15, 10, 20, 17])
        #kv[5] <= 50
